...
main:
	...
	la $s1, storeList
	la $t2, displayList
	addi $s2, $zero, 32	#the max size of refill array
	add $s4, $zero, $s2	
	addi $s3, $zero, 0	#$s3 is the counter of refill list for how many times the loop is run
	jal refill
...
refill:
	...
	beq $s3, $s4, return	#if $s3 reach the max size of the refill array, exit refill and jump to loop2 to see whether ranGen reach 32
	lb $t4, ($t2)		#
	bne $t4, -1, next
	addi $t2, $t2, 1
	j  refill
...
next:	
	sb $t4, ($s1)		#stores the extracted value into the refill array(not used)
	addi $s3, $s3, 1	#increment counter
	addi $s1, $s1, 1	#increment the address of the refill array
	addi $t2, $t2, 1
	j    refill
